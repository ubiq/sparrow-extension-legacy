...
function detectAndRunEntryTask () {
  ...
  if (!taskName) {
    throw new Error(`Sparrow build: No task name specified`)
  }
...
async function runTask (taskName, { skipStats } = {}) {
  ...
  try {
    ...
    console.error(`Sparrow build: Encountered an error while running task "${taskName}".`)
    console.error(err)
    process.exit(1)
  }
...
function createTask (taskName, taskFn) {
  if (taskName in tasks) {
    throw new Error(`Sparrow build: task "${taskName}" already exists. Refusing to redefine`)
  }
...
function runInChildProcess (task) {
  ...
  if (!taskName) {
    throw new Error(`Sparrow build: runInChildProcess unable to identify task name`)
  }
  return instrumentForTaskStats(taskName, async () => {
    ...
    await new Promise((resolve, reject) => {
      childProcess.once('close', (errCode) => {
        if (errCode !== 0) {
          reject(new Error(`Sparrow build: runInChildProcess for task "${taskName}" encountered an error`))
          return
        }
        resolve()
      })
